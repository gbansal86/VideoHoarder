...
- Built and installed `<VIDEOHOARDER_HOME>\VideoHoarder.exe`; startup and `/chatgpt-processing` HTTP verification passed. Added visible scope selectors and coverage-status filter controls. Selector data population and advanced review actions remain pending.
...
- Built release v33, installed `<VIDEOHOARDER_HOME>\VideoHoarder.exe` (218,124,389 bytes), launched it, and verified ChatGPT Processing plus selector endpoints returned HTTP 200 with every requested UI marker present. Duplicate-launch probe exited after the single-instance guard check.

## 2026-09-08 — Local canonical metadata repair from cleanup checkbox
- Added `canonical_metadata_repair` as a separate backend flag from online `metadata` refresh.
- The cleanup checkbox now triggers transcript/subtitle cleanup plus full local canonical metadata repair for already-downloaded videos.
- Local repair uses SQLite + local artifacts + retained `.info.json`, canonical normalization/persistence, migration status/version, DB backup and JSON audit.
- No YouTube API, yt-dlp refresh, or media download is invoked by local metadata repair.
- Added selected-video scoping to the metadata migration service.
- Description-file repair is still pending.

## 2026-09-08 — Governed description cleanup integration
- Added `app/description_parser.py` and `app/description_cleanup.py`.
- Cleanup checkbox now runs transcript cleanup + local canonical metadata repair + local description cleanup.
- Added automatic canonical-description gate before description-bearing planner/taxonomy package content.
- Strict transcript-intelligence package remains description-free.
- Excluded `*.comments_source.info.json` from canonical metadata repair.
- Added idempotency, safety, combined repair, package-gate, and cumulative regression tests.
- Final executed tests: 152 passed, 3 skipped.


## 2026-09-08 — Canonical description root-artifact cleanup
- Canonical description artifacts remain under each video `_data` folder.
- After `_data/description.txt_raw`, `_data/description.txt`, and `_data/description.html` are successfully created and validated, obsolete same-named copies in the video root are deleted.
- Root-level `metadata.info.json` is preserved. Retained yt-dlp `*.description` source files are preserved.
- Legacy root `description.txt_raw` is accepted as a backward-compatible source when `_data/description.txt_raw` does not yet exist, then copied into `_data` before root cleanup.
- Root cleanup is idempotent and safety failures leave legacy root files untouched.
- Focused description integration tests: 13 passed. Full repository run: 154 passed, 3 skipped, with 1 unrelated failure caused by a missing pre-existing ChatGPT integrity fixture file in the uploaded ZIP. Compile/import check passed.
