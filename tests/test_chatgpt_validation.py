...
class ChatGPTValidationTests(unittest.TestCase):
    ...
    def test_github_upload_git_lookup_uses_executable_discovery(self):
        from app import app
        with patch.object(app.shutil,"which",return_value=r"C:\Tools\Git\cmd\git.exe"):
            found=Path(app.chatgpt_git_executable())
            self.assertEqual(found,app.TOOLS/"git"/"cmd"/"git.exe")
...
def test_v334_selective_recheck_docs_match_code_contract():
    from app import app as APP
    prompt=(Path(APP.CHATGPT_MASTER_PROMPT_PATH)).read_text(encoding="utf-8")
    arch=(Path(APP.CHATGPT_ARCHITECTURE_PATH)).read_text(encoding="utf-8") if hasattr(APP,"CHATGPT_ARCHITECTURE_PATH") else (Path(APP.__file__).resolve().parents[1]/"specs"/"VIDEOHOARDER_ARCHITECTURE_V3_3_4_FINAL.md").read_text(encoding="utf-8")
    assert "Grade A: never automatically recheck online" in arch
    assert "Grade B: never automatically recheck online" in arch
    assert "default cache interval 7 days" in arch
    assert "default cache interval 3 days" in arch
    assert "default cache interval 1 day" in arch
    assert "MUST NOT broaden eligibility to A/B" in arch
    assert "Never delete or overwrite the existing transcript before" in arch
    assert "selected canonical evidence" in prompt
    assert "Do not attempt to download, search for, replace, or repair another transcript" in prompt


def test_v334_severe_timestamp_corruption_is_demoted_to_untimestamped_full_intelligence_evidence():
    import app.app as app
    transcript=(
        "[00:00:30] This transcript contains useful speech about a morning routine and practical advice. "
        "[00:00:10] The source timestamp moves backward but the sentence remains semantically useful. "
        "[00:00:20] Additional recommendations remain understandable despite timing corruption."
    )
    evidence=app.chatgpt_canonical_evidence(
        "timing_gate_1",
        {"original_title":"Timing gate regression","duration_seconds":40},
        {},transcript,"en",""
    )
    health=evidence["transcript_health"]
    assert "SEVERE_TIMESTAMP_CORRUPTION" in health["reason_codes"]
    assert evidence["transcript_available"] is True
    assert evidence["timestamps_available"] is False
    assert evidence["timestamp_status"]=="unavailable_or_unreliable"
    assert evidence["normalization_status"]=="UNTIMESTAMPED_FALLBACK"
    assert evidence["canonical_segment_count"]==0
    canonical=evidence["canonical_transcript"]
    assert canonical["segments"]==[]
    assert canonical["untimestamped_text"]
    assert canonical["timestamp_hints_available"] is True
    assert canonical["timestamp_hints_reliable"] is False
    assert canonical["source"]=="untimestamped_transcript"
    assert "authoritative reliability gate" in " ".join(evidence["source_conflicts"]).lower()


def test_v334_timestamp_reliability_gate_keeps_good_monotonic_timing_authoritative():
    import app.app as app
    transcript=(
        "[00:00:00] Opening explanation contains useful speech and enough detail for semantic processing. "
        "[00:00:10] Second point continues in chronological order with useful information. "
        "[00:00:20] Final point remains chronological and valid."
    )
    evidence=app.chatgpt_canonical_evidence(
        "timing_gate_2",
        {"original_title":"Good timing regression","duration_seconds":30},
        {},transcript,"en",""
    )
    assert evidence["transcript_available"] is True
    assert evidence["timestamps_available"] is True
    assert evidence["timestamp_status"]=="available"
    assert evidence["canonical_segment_count"]>0
    assert evidence["canonical_transcript"]["untimestamped_text"]==""


def test_v334_prompt_and_architecture_define_authoritative_timestamp_gate():
    from pathlib import Path
    root=Path(__file__).resolve().parents[1]
    prompt=(root/"app"/"prompts"/"VIDEO_LIBRARY_MASTER_PROMPT_V3_3_4_FINAL.md").read_text(encoding="utf-8")
    arch=(root/"specs"/"VIDEOHOARDER_ARCHITECTURE_V3_3_4_FINAL.md").read_text(encoding="utf-8")
    assert "explicit VideoHoarder reliability assertion" in prompt
    assert "timestamps ALONE do not qualify" in prompt
    assert "Authoritative Timestamp Reliability Gate" in arch
    assert "SEVERE_TIMESTAMP_CORRUPTION" in arch
    assert "TIMESTAMP_PARSE_WEAK" in arch
    assert "canonical_transcript.segments = []" in arch


def test_v334_rolling_youtube_vtt_collapses_progressive_display_overlap():
    import app.app as app
    vtt=(
        "WEBVTT\n\n"
        "00:00:01.000 --> 00:00:02.000\nToday we\n\n"
        "00:00:02.000 --> 00:00:03.000\nToday we will make\n\n"
        "00:00:03.000 --> 00:00:04.000\nToday we will make oats tikki\n"
    )
    out=app.vtt_to_timestamped_transcript(vtt,marker_interval=15)
    clean=app.timestamped_to_clean(out)
    assert clean.strip()=="Today we will make oats tikki"
    assert clean.lower().count("today")==1
    assert clean.lower().count("will make")==1


def test_v334_large_unsubtitled_tail_is_not_duration_only_truncation():
    import app.app as app
    transcript=(
        "[00:00:00] This opening contains useful spoken content and enough semantic detail. "
        "[00:05:20] This final spoken sentence ends naturally and contains useful information."
    )
    health=app.chatgpt_transcript_health(transcript,600,{})
    assert "LARGE_UNSUBTITLED_TAIL" in health["reason_codes"]
    assert "END_COVERAGE_UNCERTAIN" in health["reason_codes"]
    assert "CLEAR_MAJOR_TRUNCATION" not in health["reason_codes"]
    assert "CONFIRMED_MISSING_SPOKEN_CONTENT" not in health["reason_codes"]
    assert health["hard_failure"] is False
    evidence=app.chatgpt_canonical_evidence("tail1",{"duration_seconds":600},{},transcript,"en","")
    assert evidence["transcript_completeness"]=="UNKNOWN"
    assert evidence["transcript_usability"]=="USABLE"


def test_v334_independent_missing_speech_proof_marks_partial_hard_failure():
    import app.app as app
    transcript=(
        "[00:00:00] This opening contains useful spoken content and enough semantic detail. "
        "[00:05:20] This available portion ends here but remains semantically useful."
    )
    manifest={"confirmed_missing_spoken_content":True}
    health=app.chatgpt_transcript_health(transcript,600,manifest)
    assert "CONFIRMED_MISSING_SPOKEN_CONTENT" in health["reason_codes"]
    assert health["hard_failure"] is True
    assert health["routing_class"]=="REPAIR_REQUIRED"
    evidence=app.chatgpt_canonical_evidence("tail2",{"duration_seconds":600},manifest,transcript,"en","")
    assert evidence["transcript_completeness"]=="PARTIAL"
    assert evidence["transcript_available"] is True


def test_v334_character_count_alone_never_proves_transcript_complete():
    import app.app as app
    # Long text with only an early timestamp is usable but cannot establish that
    # all meaningful speech in a one-hour video is represented.
    transcript="[00:00:00] "+("Detailed useful spoken explanation with meaningful information. "*1800)
    evidence=app.chatgpt_canonical_evidence("long1",{"duration_seconds":3600},{},transcript,"en","")
    assert evidence["transcript_available"] is True
    assert evidence["transcript_characters"]>5000
    assert evidence["transcript_completeness"]=="UNKNOWN"
    assert evidence["transcript_completeness"]!="COMPLETE"


def test_v334_near_edge_timing_is_likely_complete_not_absolute_complete():
    import app.app as app
    transcript=(
        "[00:00:01] This opening has useful spoken content with enough semantic information. "
        "[00:09:50] This final spoken point ends close to the end of the video."
    )
    evidence=app.chatgpt_canonical_evidence("near1",{"duration_seconds":600},{},transcript,"en","")
    assert evidence["transcript_completeness"]=="LIKELY_COMPLETE"


def test_v334_prompt_and_architecture_define_speech_coverage_boundary():
    root=Path(__file__).resolve().parents[1]
    prompt=(root/"app"/"prompts"/"VIDEO_LIBRARY_MASTER_PROMPT_V3_3_4_FINAL.md").read_text(encoding="utf-8")
    arch=(root/"specs"/"VIDEOHOARDER_ARCHITECTURE_V3_3_4_FINAL.md").read_text(encoding="utf-8")
    assert "SOURCE-COVERAGE AND CAPTION-ARTIFACT SEMANTIC CLARIFICATION" in prompt
    assert "CAPTION DISPLAY ARTIFACTS ARE NOT SPOKEN EMPHASIS" in prompt
    assert "VIDEO DURATION != SPEECH DURATION" in arch
    assert "LARGE_UNSUBTITLED_TAIL" in arch
    assert "CONFIRMED_MISSING_SPOKEN_CONTENT" in arch
    assert "USABLE` is not a completeness state" in arch


def _sample_vtt(text, start='00:00:00.000', end='00:00:08.000'):
    return f"WEBVTT\n\n{start} --> {end}\n{text}\n"


def test_v334_best_vtt_selection_is_health_language_scored_not_alphabetic(tmp_path):
    import app.app as app
    # Alphabetically en.vtt would normally come before en-orig.vtt, but the
    # policy must use semantic/source scoring rather than name order.
    plain=tmp_path/'abc.en.vtt'
    orig=tmp_path/'abc.en-orig.vtt'
    hi=tmp_path/'abc.hi.vtt'
    plain.write_text(_sample_vtt('This English caption contains enough useful spoken words for a valid transcript source.'),encoding='utf-8')
    orig.write_text(_sample_vtt('This original English caption contains enough useful spoken words and a little more source detail for selection.'),encoding='utf-8')
    hi.write_text(_sample_vtt('यह हिंदी कैप्शन उपयोगी बोली गई सामग्री के लिए पर्याप्त शब्द रखता है और एक वैध स्रोत है।'),encoding='utf-8')
    winner=app.select_best_vtt_candidate([plain,hi,orig],'abc')
    assert winner is not None
    assert winner['path']==orig
    assert winner['lang']=='en'


def test_v334_persistent_cleanup_leaves_exactly_one_vtt_one_srt_one_transcript(tmp_path):
    import app.app as app
    folder=tmp_path/'video'
    data=folder/'_data';data.mkdir(parents=True)
    vid='abc123'
    best=data/f'{vid}.en-orig.vtt'
    extra=data/f'{vid}.en.vtt'
    locale=data/f'{vid}.en-US.vtt'
    legacy_ts=data/'Old Title.transcript_timestamped.txt'
    legacy_clean=data/'Old Title.transcript_clean.txt'
    best.write_text(_sample_vtt('This original English subtitle has enough useful spoken words and is the selected canonical source for the video.'),encoding='utf-8')
    extra.write_text(_sample_vtt('This English subtitle has enough useful spoken words but is an inferior duplicate source for this video.'),encoding='utf-8')
    locale.write_text(_sample_vtt('This locale English subtitle has enough useful spoken words but should not remain after canonical cleanup.'),encoding='utf-8')
    legacy_ts.write_text('[00:00:00] old timestamped transcript duplicate that should be removed after verification',encoding='utf-8')
    legacy_clean.write_text('old clean transcript duplicate that should be removed after verification',encoding='utf-8')

    result=app.canonicalize_video_subtitle_artifacts(folder,vid,update_db=False)
    assert result['ok'] is True
    assert (data/f'{vid}.en.vtt').is_file()
    assert (data/f'{vid}.srt').is_file()
    assert (data/f'{vid}.transcript.txt').is_file()
    assert len(list(folder.rglob('*.vtt')))==1
    assert len(list(folder.rglob('*.srt')))==1
    persistent_transcripts=[p for p in data.glob('*transcript*.txt') if 'comments' not in p.name.lower()]
    assert persistent_transcripts==[data/f'{vid}.transcript.txt']
    assert app.transcript_has_useful_speech((data/f'{vid}.transcript.txt').read_text(encoding='utf-8'))
    assert (data/'subtitle_cleanup.audit.jsonl').is_file()


def test_v334_cleanup_failure_never_deletes_existing_subtitle_variants(tmp_path):
    import app.app as app
    folder=tmp_path/'video';data=folder/'_data';data.mkdir(parents=True)
    vid='bad123'
    one=data/f'{vid}.en.vtt';two=data/f'{vid}.en-orig.vtt'
    one.write_text(_sample_vtt('[Music]'),encoding='utf-8')
    two.write_text(_sample_vtt('[Applause]'),encoding='utf-8')
    result=app.canonicalize_video_subtitle_artifacts(folder,vid,update_db=False)
    assert result['ok'] is False
    assert one.exists() and two.exists()
    assert len(list(folder.rglob('*.vtt')))==2
    assert len(list(folder.rglob('*.srt')))==0


def test_v334_docs_define_exact_persistent_three_file_contract():
    root=Path(__file__).resolve().parents[1]
    prompt=(root/'app'/'prompts'/'VIDEO_LIBRARY_MASTER_PROMPT_V3_3_4_FINAL.md').read_text(encoding='utf-8')
    arch=(root/'specs'/'VIDEOHOARDER_ARCHITECTURE_V3_3_4_FINAL.md').read_text(encoding='utf-8')
    assert 'PERSISTENT SUBTITLE / TRANSCRIPT FILE CONTRACT' in arch
    assert '<VIDEO_ID>.<best-language>.vtt' in arch
    assert '<VIDEO_ID>.srt' in arch
    assert '<VIDEO_ID>.transcript.txt' in arch
    assert 'Cleanup is transactional' in arch and 'MUST NOT report success while extra VTT/SRT/transcript variants remain' in arch
    assert 'canonical transcript generated from exactly' in prompt and 'one selected winning subtitle source' in prompt


def test_v334_canonical_srt_uses_deduplicated_rolling_caption_sequence(tmp_path):
    import app.app as app
    folder=tmp_path/'video';data=folder/'_data';data.mkdir(parents=True)
    vid='roll123'
    vtt=data/f'{vid}.en.vtt'
    vtt.write_text('''WEBVTT\n\n00:00:00.000 --> 00:00:01.000\nToday we\n\n00:00:01.000 --> 00:00:02.000\nToday we will make\n\n00:00:02.000 --> 00:00:03.000\nToday we will make oats tikki\n\n00:00:03.000 --> 00:00:04.000\nToday we will make oats tikki with protein\n''',encoding='utf-8')
    result=app.canonicalize_video_subtitle_artifacts(folder,vid,update_db=False)
    assert result['ok'] is True
    srt=(data/f'{vid}.srt').read_text(encoding='utf-8')
    compact=' '.join(srt.split())
    assert 'Today we will make oats tikki with protein' in compact
    assert compact.count('Today') == 1


def test_v334_cleanup_removes_legacy_transcript_txt_outside_data(tmp_path):
    import app.app as app
    folder=tmp_path/'video';data=folder/'_data';data.mkdir(parents=True)
    vid='outside123'
    (data/f'{vid}.en.vtt').write_text(_sample_vtt('This is a sufficiently useful English subtitle source for cleanup verification and transcript generation.'),encoding='utf-8')
    legacy=folder/'Old Title.transcript_timestamped.txt'
    legacy.write_text('[00:00:00] This old duplicate transcript should be removed from the video folder.',encoding='utf-8')
    result=app.canonicalize_video_subtitle_artifacts(folder,vid,update_db=False)
    assert result['ok'] is True
    assert not legacy.exists()
    transcripts=[p for p in folder.rglob('*.txt') if 'transcript' in p.name.lower() and 'comments' not in p.name.lower()]
    assert transcripts == [data/f'{vid}.transcript.txt']


def test_v334_cleanup_reports_failure_and_restores_variants_if_removal_fails(tmp_path,monkeypatch):
    import app.app as app
    folder=tmp_path/'video';data=folder/'_data';data.mkdir(parents=True)
    vid='rollback123'
    best=data/f'{vid}.en-orig.vtt';extra1=data/f'{vid}.en.vtt';extra2=data/f'{vid}.en-US.vtt'
    best.write_text(_sample_vtt('This original English subtitle contains enough useful spoken words and additional detail to win source selection.'),encoding='utf-8')
    extra1.write_text(_sample_vtt('This English subtitle contains enough useful spoken words but is an inferior duplicate candidate.'),encoding='utf-8')
    extra2.write_text(_sample_vtt('This locale subtitle contains enough useful spoken words but is another inferior duplicate candidate.'),encoding='utf-8')
    real_move=app.shutil.move
    calls={'n':0}
    def flaky_move(src,dst,*args,**kwargs):
        # Fail exactly once while staging the second inferior artifact; allow rollback.
        if 'subtitle_cleanup_quarantine' not in str(src):
            calls['n']+=1
            if calls['n']==2:
                raise PermissionError('simulated locked subtitle file')
        return real_move(src,dst,*args,**kwargs)
    monkeypatch.setattr(app.shutil,'move',flaky_move)
    result=app.canonicalize_video_subtitle_artifacts(folder,vid,update_db=False)
    assert result['ok'] is False
    # No success may be reported with partially removed old variants.
    assert extra1.exists() and extra2.exists()
    assert 'cleanup failed' in result['reason'].lower()


def test_v334_missing_data_gui_exposes_explicit_subtitle_cleanup_control():
    import app.app as app
    import inspect
    ...
    assert 'if options.get("subtitle_cleanup")' in src
