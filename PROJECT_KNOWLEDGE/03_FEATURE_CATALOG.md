...
| Native dashboard/navigation | IMPLEMENTED / PARTIAL | `app/native_ui.py`, `app/native_queue_page.py`, `Sidebar`, `CommandCenter`, `WorkflowsPage`, `SettingsPage` | Job Details now renders a live child-video table without changing the main layout; PySide runtime tests remain skipped where PySide6 is unavailable |
...
| Download pipeline | IMPLEMENTED / LIVE VERIFIED | `process_download()`, full/media-only/external/legacy bounded schedulers, `add_download_performance_args()` | Two-video 360p live run succeeded with overlapping metadata and media execution, two valid MP4s, per-video progress, and 0 failures |
| Queue, progress, cancellation | IMPLEMENTED / LIVE VERIFIED | `web_full_download()`, `web_video_progress()`, `JobContext`, queue/details UI | Bounded schedulers preserve order/ownership; child percent/speed/ETA/status is displayed live. Managed live cancellation and same-job retry completed 2/2 successfully; fallback attempts now short-circuit on cancellation |
