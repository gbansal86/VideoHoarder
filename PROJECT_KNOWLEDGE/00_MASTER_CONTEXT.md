...
Source: `<VIDEOHOARDER_HOME>\Source`. Installed runtime: `<VIDEOHOARDER_HOME>\VideoHoarder.exe`. Git: private `https://github.com/gbansal86/VideoHoarder.git`, branch `main`.
...
Safety: ChatGPT exchange is manual; imported data requires validation/review; duplicate and clip review do not make physical changes; separate destructive tools require explicit use. Plaintext `<VIDEOHOARDER_HOME>\api_key.txt` exists and must never enter Git, reports or handoffs.
