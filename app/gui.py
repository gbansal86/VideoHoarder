...
from .native_library_page import NativeLibraryPage
from .native_failure_page import NativeFailurePage
from .native_queue_page import NativeQueuePage


def application_root() -> Path:
    """Return a writable persistent app root in source and frozen builds."""
    if getattr(sys, "frozen", False):
        release_root = Path(sys.executable).resolve().parent
        portable_data = release_root / "Downloads"
        if portable_data.is_dir():
            return portable_data
        program_roots = [os.environ.get("ProgramFiles"), os.environ.get("ProgramFiles(x86)"), os.environ.get("ProgramW6432")]
        target = str(release_root).lower()
        if any(root and (target == str(Path(root).resolve()).lower() or target.startswith(str(Path(root).resolve()).lower() + os.sep.lower())) for root in program_roots):
            local = Path(os.environ.get("LOCALAPPDATA") or (Path.home() / "AppData" / "Local")) / "VideoHoarder"
...
class MainWindow(QMainWindow):
    ...
    LEGACY_ROUTES = {
        "downloads": ("Downloads", "/app?tab=downloads"),
        "oldimport": ("Old Library Import & Repair", "/oldimport"),
        "localimport": ("Import Local Videos", "/import-local"),
        ...
        "subscriptions": ("Subscriptions", "/app?tab=subscriptions"),
    ...
    def __init__(self, log_path: Path) -> None:
        ...
        self.command_center.navigate_requested.connect(self.show_page)
        ...
        self.library_page = NativeLibraryPage()
        self.library_page.open_video_requested.connect(self.open_library_video)
        self.library_page.status_message.connect(self._show_status)
        self.failure_page = NativeFailurePage()
        self.failure_page.status_message.connect(self._show_status)
        self.failure_page.failure_count_changed.connect(self.command_center.set_failure_count)
        self.queue_page = NativeQueuePage()
        self.queue_page.status_message.connect(self._show_status)
        self.queue_page.open_folder_requested.connect(self.open_downloads_folder)
        ...
        right = QWidget()
        right.setObjectName("shellContent")
        right_layout = QVBoxLayout(right)
        right_layout.setContentsMargins(0, 0, 0, 0)
        right_layout.setSpacing(0)
        right_layout.addWidget(self.content_stack, 1)
        self.global_notice = QLabel("Ready")
        self.global_notice.setObjectName("globalNotice")
        self.global_notice.setWordWrap(False)
        self.global_notice.setMinimumHeight(34)
        self.global_notice.setToolTip("Latest application message")
        right_layout.addWidget(self.global_notice)
    ...
    def backend_ready(self, base_url: str, server: Any, backend: Any) -> None:
        ...
        # Queue is attached first so a slow/failing secondary page cannot hide
        # a job that was successfully submitted to the backend. Each page has
        # its own failure boundary and the exact attachment error is logged.
        attach_errors: list[str] = []
        for page_name, page in (
            ("queue", self.queue_page),
            ("dashboard", self.command_center),
            ("workflows", self.workflows_page),
            ("settings", self.settings_page),
            ("library", self.library_page),
            ("failures", self.failure_page),
        ):
            try:
                page.set_backend(backend)
            except Exception:
                detail = f"{page_name}: {traceback.format_exc()}"
                print(f"Backend attachment failed for {detail}", file=sys.stderr)
                attach_errors.append(page_name)
        self.show_page("dashboard" if "dashboard" not in attach_errors else "queue")
        if attach_errors:
            self._show_status(
                f"Library ready with page warning(s): {', '.join(attach_errors)} · Queue remains available"
            )
        else:
    ...
    def show_page(self, key: str) -> None:
        ...
        if key == "status":
            key = "failures"
        focus_downloader = key == "dashboard_download"
        if focus_downloader:
            key = "dashboard"
        ...
        if key == "dashboard":
            ...
            self.command_center.set_mode("dashboard")
            self.command_center.refresh_failure_count()
            self.command_center.refresh_stats()
            if focus_downloader:
                QTimer.singleShot(0, self.command_center.focus_downloader)
            return
        if key == "queue":
            self.content_stack.setCurrentWidget(self.queue_page)
            self.queue_page.activate()
            return
        if key == "library":
            ...
            return
        if key == "failures":
            self.content_stack.setCurrentWidget(self.failure_page)
            self.failure_page.activate()
            return
        if key == "more":
    ...
    @Slot(str)
    def open_library_video(self, video_id: str) -> None:
        """Play a local Library video in the embedded browser player."""
        import urllib.parse

        vid = str(video_id or "").strip()
        if not vid:
            return
        self.web_title.setText("Library video")
        self.content_stack.setCurrentWidget(self.web_panel)
        self.navigate(f"/video-view/{urllib.parse.quote(vid, safe='')}")
    ...
    @Slot(object)
    def _settings_saved(self, values: object) -> None:
        self.command_center.apply_settings_defaults(dict(values or {}), force=False)
    ...
    def _show_status(self, message: str) -> None:
        ...
        self.global_notice.setText(text[:180])
        self.global_notice.setToolTip(text)
